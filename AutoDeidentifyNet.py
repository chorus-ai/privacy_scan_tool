...
import pickle
with open('./phi_scan/whitelist.pkl', 'rb') as f:
    l = pickle.load(f)
...
pattern_race = re.compile(r"""
(White/Caucasian|White|Caucasian|American|Indian|Alaska|Native|Asian|Black|African|Native|Hawaiian|Pacific|Islander)""", re.X)

# match race/ethnicity
pattern_sex = re.compile(r"""(Male|Female)""", re.X)


# match race/ethnicity
pattern_ethnicity = re.compile(r"""(Hispanic|Latino)""", re.X)
...
def filter_task(note, whitelist_dict):
    N = note.count(',')
...
#         print('sent is', sent)
    ...
    if pattern_race.findall(sent) != []:
        safe = False
        for item in pattern_race.findall(sent):
            screened_words.append(item[0])
    sent = str(pattern_race.sub('**PHI**', sent))
    
    if pattern_ethnicity.findall(sent) != []:
        safe = False
        for item in pattern_ethnicity.findall(sent):
            screened_words.append(item[0])
    sent = str(pattern_ethnicity.sub('**PHI**', sent))

    if pattern_sex.findall(sent) != []:
        safe = False
        for item in pattern_sex.findall(sent):
            screened_words.append(item[0])
    sent = str(pattern_sex.sub('**PHI**', sent))
...
#     print('phi_reduced is ',phi_reduced)
    n_phi = phi_reduced.count('PHI')
    prob = 1.0 if n_phi/N > 1.0 else n_phi/N
    return n_phi/N
