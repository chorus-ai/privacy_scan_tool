...
if __name__=='__main__': 
    ...
    parser = argparse.ArgumentParser(description='De-identification')

    parser.add_argument('--original_data_path', nargs='?', help='')
    parser.add_argument('--json_file_path', nargs='?', help='')
    parser.add_argument('--model_path', nargs='?', help='')
    parser.add_argument('--output_file_name', nargs='?', help='')
    ...
    df_data = pd.read_csv(args.original_data_path)
    print('Finish loading data 1')

    
    # load model
    model_xgb = XGBClassifier()
    model_xgb.load_model(args.model_path)
    
    csv_result = main(model_xgb, df_data, json_data)
#     print(args.output_file_name)
    csv_result.to_csv(args.output_file_name[:-4] + str(datetime.datetime.now())+'.csv')
    print('Result saved to Prediction_result: ' + args.output_file_name + str(datetime.datetime.now()) + '.csv')
